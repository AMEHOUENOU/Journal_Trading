<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">Modifier la note — {{ \Carbon\Carbon::parse($note->date)->format('d/m/Y') }}</h2>
    </x-slot>

    <div class="py-8 bg-gray-950 min-h-screen">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <form method="POST" action="{{ route('daily-notes.update', $note) }}">
                    @csrf
                    @method('PUT')

                    <div class="mb-4">
                        <label class="block text-sm text-gray-300 mb-1">Date</label>
                        <input type="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($note->date)->format('Y-m-d')) }}" required
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" rows="8"
                            class="w-full bg-gray-950 border border-gray-800 text-gray-200 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-0">{{ old('content', $note->content) }}</textarea>
                    </div>
