<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Modifier {{ $account->name }}</h2>
    </x-slot>
            ...
            <div class="bg-white shadow rounded-lg p-6">
                <form method="POST" action="{{ route('accounts.update', $account) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="broker" name="broker" type="text" class="mt-1 block w-full" :value="old('broker', $account->broker)" />
                    </div>

                    <div class="mb-4">
                        <x-input-label for="current_balance" value="Balance actuelle" />
                        <x-text-input id="current_balance" name="current_balance" type="number" step="0.01" class="mt-1 block w-full" :value="old('current_balance', $account->current_balance)" required />
                    ...
                    <div class="mb-6">
                        ...
                        <select id="currency" name="currency" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach (['USD', 'EUR', 'XOF'] as $currency)
